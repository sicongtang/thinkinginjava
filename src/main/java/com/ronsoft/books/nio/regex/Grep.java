...
public class Grep {
	 ...
	 * different Java NIO 176 methods. The printed ouptut is slightly different
	 * when -1 is specified.
	 */
		...
		if (onebyone) {
			...
			while (it.hasNext()) {
				...
				for (int i = 0; i < matches.length; i++) {
					...
					System.out.println("  " + match.getLineNumber() + " [" + match.start() + "-" + (match.end() - 1)
							+ "]: " + match.getLineText());
				}
			}
		} else {
			...
			for (int i = 0; i < matches.length; i++) {
				MatchedLine match = matches[i];
				System.out.println(match.getFile().getName() + ", " + match.getLineNumber() + ": "
						+ match.getLineText());
			}
		}
	}
